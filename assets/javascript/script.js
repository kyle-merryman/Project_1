...
$("#modal-login").click(function(event) {
  ...
  database.ref("/accounts/").on(
    "child_added",
    function(snapshot) {
      var fireBaseAccount = snapshot.val();
      if (
        fireBaseAccount.username === loginUsername &&
        fireBaseAccount.password === loginPassword
      ) {
        userIsLoggedIn = true;
      } else {
        console.log("stop it");
...
setTimeout(function() {
  ...
  for (i = 0; i < eventArray.length; i++) {
    ...
    var eventWeather = database.ref(
      "/calendarEvents/" + [i] + "/postData/eventWeather/"
    ); //it's EITHER "[i]" OR "this", not both?
